<template>
  <el-table v-bind="$attrs" :stripe="true" style="width: 100%">
    ...
    <el-table-column prop="transport_status_label" label="订单状态" width="130">
      <template slot-scope="scope">
        <el-select
          v-if="scope.row.isEdit"
          v-model="scope.row.tempTransportStatusLabel"
          clearable
          @change="
            (val) => {
              transportStatusChange(val, scope);
            }
          "
          placeholder="请选择"
        >
        ...
        <div v-else>{{ scope.row.transport_status_label }}</div>
      </template>
    </el-table-column>
  ...
  methods: {
    ...
    transportStatusChange(val, scope) {
      ...
      for (let item of this.transportStatusOptions) {
        if (item.status_name == val) {
          scope.row.tempTransportStatus = item.id;
          break;
        }
      }
    },
