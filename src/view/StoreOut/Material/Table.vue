<template>
  <el-table v-bind="$attrs" :stripe="true" style="width: 100%">
    ...
    <!-- <el-table-column prop="price" label="单价" width="120">
      <template slot-scope="scope">
        <el-input
          v-if="scope.row.isEdit && scope.row.out_type == 1"
          v-model="scope.row.tempPrice"
        ></el-input>
        <div v-else>{{ scope.row.price ? scope.row.price : "/" }}</div>
      </template>
    </el-table-column> -->
    <el-table-column prop="client_name" label="客户" width="120">
      <template slot-scope="scope">
        ...
        ></el-autocomplete>
        <div v-else>
          {{ scope.row.client_name ? scope.row.client_name : "/" }}
        </div>
      </template>
    </el-table-column>
    <el-table-column prop="out_type" label="出库类型" width="120">
      <template slot-scope="scope">
        <span v-if="scope.row.out_type == 1" style="color: red">退货</span>
        <span v-else style="color: #e6a23c">损坏</span>
      </template>
    </el-table-column>
    <el-table-column prop="updata_date" label="日期" width="240">
      <template slot-scope="scope">
        <el-date-picker
          v-if="scope.row.isEdit"
