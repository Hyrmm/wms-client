...
export default {
  ...
  computed: { ...mapState("cache", ["materialOutStoreCache"]) },
  methods: {
    postRecording() {
      this.post("outStore", [
        "stock_id",
    ...
    editRow(rowData) {
      if (rowData.isEdit == undefined) {
        ...
        this.$set(rowData, "tempOutType", rowData.out_type);
    ...
    async post() {
      let finishCache = [];
      for await (let row of this.materialOutStoreCache) {
        if (row.checked) {
          try {
            let res = await outStoreMaterial({
      ...
      for (let item of finishCache) {
        this.del_outStoreCache({
          index: this.materialOutStoreCache.indexOf(item),
          stockType: 1,
        });
      }
    },
